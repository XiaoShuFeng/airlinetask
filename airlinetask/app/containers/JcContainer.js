...
class JcContainer extends React.Component {
    ...
    _getBatchDataWithIds = (key,ids,callback,fail)=>{
        key = key.replace(/:/g,"/")
        console.log('loadbatched Data')
        console.log(key)
        console.log(ids)
        SoraModule.getBatchedData(key,"",ids.join(","),data=>{
            // console.log(data);
            if(typeof(callback) === 'function'){
                callback(data);
            }
    ...
    _remove = (key,callback,fail) =>{
        key = key.replace(/:/g,"/")
        console.log('删除文件/目录 :' + key)
        SoraModule.removeDirectory(key,data=>{
            if(typeof(callback) === 'function'){
                callback(data);
            }
        },err=>{
    ...
    esignOperation = (val) => {
        ...
        if (dataArr[0] === 'esign' || dataArr[0] === 'modifyesign') {
                ...
                this.setState({
                    ...
                    if(Platform.OS === 'android'){
                        ...
                        if(!this.state.online){
                            ...
                            that._getBatchDataWithIds(this.state.userId + ':' + that.state.taskId,ids,(ret)=>{
                                let jcdataArr = []
                                for(let i =0;i<ret.length;i++){
                                    jcdataArr.push(JSON.parse(ret[i]).jcdata)
                                }
                                console.log('show')
                                ...
                                that.refs.webViewRef.postMessage('error:' + dataArr[3])
                            })
            ...
            if(dataArr[1] === 'jcid'){
                ...
                signId = dataArr[2]
                location = dataArr[3]
                that._loadDataFile(this.state.userId + ':' + that.state.taskId,'jcpossign-'+this.state.jcId+'-'+location,(ret)=>{
                    console.log('删除已有签名 ')
                    ret = JSON.parse(ret)
                    ret.base64 = undefined
                    that._saveDataFile(this.state.userId + ':' + that.state.taskId,'jcpossign-'+this.state.jcId+'-'+location,JSON.stringify(ret))
                    that._remove(this.state.userId+':offlinepost:'+that.state.taskId+':offlinepost'+that.state.jcId+location,(data)=>{
                        console.log('删除结果 成功：'+data)
                    },err=>{
                        console.log('删除结果 失败：'+err)
                    })
                    // 删除请求post内容
                    // storage.remove({
            ...
            let userid = dataArr[1]
            let jcid = dataArr[2]
            this._loadDataFile(userid + ':' +that.state.taskId,'jcinfo'+jcid,(ret)=>{
            ...
            console.log(ids)
            ids = ids.split(',')
            that._getBatchDataWithIds(userid+':'+that.state.taskId,ids,(ret)=>{
                console.log(ret)
                let response = {}
                for(let i=0; i<ret.length; i++){
                    response[JSON.parse(ret[i]).location] = JSON.parse(ret[i])
                }
                this.refs.webViewRef.postMessage('offlinepossignresponse:'+JSON.stringify(response))
            },(err)=>{
                this.refs.webViewRef.postMessage('offlinepossignresponse:'+JSON.stringify({}))
                console.log(err)
            })
            ...
            let userid = dataArr[1]
            let jcid = dataArr[2]
            this._loadDataFile(userid + ':' +that.state.taskId,'jccomplish-'+jcid,(ret)=>{
                ret = JSON.parse(ret)
            ...
            let userid = dataArr[1]
            let jcid = dataArr[2]
            this._loadDataFile(userid +  ':' + that.state.taskId,'jccomplishverify-'+jcid,(ret)=>{
                ret = JSON.parse(ret)
            ...
            let userid = dataArr[1]
            let jcid = dataArr[2]
            this._loadDataFile(userid + ':' + that.state.taskId,'jccomplishmodifyverify-'+jcid,(ret)=>{
                console.log('---------------------修改完工')
                ret = JSON.parse(ret)
                console.log(ret)
            ...
            console.log(ids)
            ids = ids.split(',')
            that._getBatchDataWithIds(userid+':'+that.state.taskId+':input',ids,(ret)=>{
                console.log(ret)
                let response = {}
                for(let i=0; i<ret.length; i++){
                    response[ids[i]] = JSON.parse(ret[i])
                }
            ...
            console.log(ids)
            ids = ids.split(',')
            that._getBatchDataWithIds(userid+':'+that.state.taskId+':inputchange',ids,(ret)=>{
                console.log(ret)
                let response = {}
                for(let i=0; i<ret.length; i++){
                    response[ids[i]] = ret[i]
            ...
            this._saveDataFile(userid+':'+that.state.taskId+':input',dbid,JSON.stringify(data),'离线存储Input框数据');
            console.log(data)
            ...
            this._saveDataFile(userid+':offlineinput:'+this.state.taskId,dbid,JSON.stringify(data),'离线存储Input框请求的数据');
    ...
    componentDidMount(){
            ...
            if(this.state.signType === 'date'){  // 日期签署
                ...
                if(this.state.location.indexOf(',') >0){
                    ...
                    for(let i=0;i<locationArr.length;i++){
                        ...
                        that._saveDataFile(this.state.userId+':'+that.state.taskId,'jcpossign-'+this.state.jcId+'-'+locationArr[i],JSON.stringify(signInfo))
                        this.refs.webViewRef.postMessage('base64:'  + signidArr[i] + ':' + msg + ':' + locationArr[i])
                    }
                }else{
                    // 单签
                    that._loadDataFile(this.state.userId + ':' + that.state.taskId,'jcpossign-'+this.state.jcId+'-'+this.state.location,(ret)=>{
                        console.log("修改已有")
                        ret = JSON.parse(ret)
                        ret.base64 = msg
                        that._saveDataFile(this.state.userId + ':' + that.state.taskId,'jcpossign-'+this.state.jcId+'-'+this.state.location,JSON.stringify(ret))
                        ...
                        that._saveDataFile(this.state.userId + ':' + that.state.taskId,'jcpossign-'+this.state.jcId+'-'+this.state.location,JSON.stringify(signinfo))
                        this.refs.webViewRef.postMessage('base64:'  + this.state.signId + ':' + msg + ':' + this.state.location)
                    })
                }
        ...
        DeviceEventEmitter.addListener('AndroidToRNMessageOffline',(msg)=>{
            ...
            console.log('接受 手机发送的 离线签名请求包')
            console.log('当前的taskId:'+this.state.taskId)
            console.log('当前是location:' + that.state.location)
            let locationArr = that.state.location.split(',')
            let postArr = msg.split('-split-')
            for(let i=0;i<locationArr.length;i++){
                that._saveDataFile(this.state.userId+':offlinepost:'+this.state.taskId,'offlinepost'+that.state.jcId+locationArr[i],postArr[i])
            }
