...
class HomeContainer extends React.Component {
    ...
    _getAllDataForKey = (key,callback,fail)=>{
        key = key.replace(/:/g,"/")
        SoraModule.getBatchedDataByKey(key,(data)=>{
            console.log(data)
            if(typeof(callback) === 'function'){
                callback(data)
            }
        ...
        for ( let i=0; i < neededitemlist.length; i++ ) {
            ...
            if(neededitemlist[i].dataType === 'TD航线' && neededitemlist[i].jcId !=='' && neededitemlist[i].jcId !==undefined && neededitemlist[i].taskState === 'on-work' && neededitemlist[i].layoverJcId !==undefined) {
                ...
                axios.get( that.state.url + '/xmis/LMJcAction/GetJcHtmlPage.do?jcid='+neededitemlist[i].layoverJcId+'&esign=1').then(response => {
                    ...
                    // 存储电签html语句
                    that._saveDataFile(userid+':'+taskid,'jcpage'+neededitemlist[i].layoverJcId,response.data,'离线存储html语句')
                    ...
                    axios.post( that.state.url + '/xmis/LMJcAction/GetJcInfo.do?jcid='+neededitemlist[i].layoverJcId).then(jcinforesponse =>{
                        ...
                        if (i == neededitemlist.length-1) {
                            ...
                            if(checkAllFinish() && i=== neededitemlist.length-1 ){
                        ...
                        that._saveDataFile(userid+':'+taskid,'jcinfo'+neededitemlist[i].layoverJcId,JSON.stringify(jcinforesponse.data),'存储jcinfo')
                        ...
                        console.log(err)
                        that._removeDownloaded(userid,taskid,(data)=>{
                        ...
                        axios.post( that.state.url + '/xmis/LMJcAction/GetJcPosSign.do?jcid='+neededitemlist[i].layoverJcId+'&location='+location).then(postionresponse =>{    
                            ...
                            if (postionresponse.data.base64) {
                                ...
                                axios.post( that.state.url + '/xmis/LMJcAction/VerifyJcPosSign.do?jcid='+jcid+'&location='+location).then(res=>{
                                    ...
                                    that._saveDataFile(userid+':'+taskid,'jcpossignverify:'+jcid+'-'+location,JSON.stringify(res.data))
                                }).catch(err=>{
                            ...
                            that._saveDataFile(userid+':'+taskid,'jcpossign-'+jcid+'-'+location,JSON.stringify(postionresponse.data))
                            ...
                            axios.get( that.state.url + '/xmis/AirlineTaskAction/getJcData.do?signtype=pos&jcid='+jcid+'&posid='+postionresponse.data.posid+'&signid='+postionresponse.data.signid+'&userid='+userid).then(res=>{
                                ...
                                // console.log(res.data)
                                that._saveDataFile(userid+':'+taskid,'jcdata-'+jcid+'-'+postionresponse.data.posid+'-'+postionresponse.data.signid,JSON.stringify(res.data),'存储离线jcdata')
                    ...
                    for(let k=0;k<imgarr.length;k++){
                        axios.get( that.state.url + '/xmis/LMJcAction/GetJcImage.do?n='+imgarr[k],{responseType:'blob'}).then(res=>{
                            ...
                            oFileReader.onloadend = function (e) {
                                ...
                                that._saveDataFile(userid+':'+taskid,imgarr[k],base64,'离线存储网页里的图片')
                    ...
                    for(let l=0;l<inputarr.length;l++){
                        ...
                        axios.post( that.state.url + '/xmis/LMJcAction/GetJcHtmlInputValue.do?jcid='+jcid+'&dbid='+dbid).then(r=>{
                            ...
                            that._saveDataFile(userid+':'+taskid+':input',dbid,JSON.stringify(r.data),'离线存储Input框数据')
                            ...
                            axios.post( that.state.url + '/xmis/LMJcAction/VerifyInputValue.do?jcid='+jcid+'&userid='+userid+'&dbid='+dbid+'&value='+r.data.value).then(rsponse=>{
                                ...
                                that._saveDataFile(userid+':'+taskid+':inputchange',dbid,JSON.stringify(rsponse.data),'离线存储是否能修改Input框数据')
                            ...
                            that._removeDownloaded(userid,taskid)
                            that.refs.webViewRef.postMessage("downloadFinish:no:"+taskid)
                            that._toast('下载失败：下载签名页的输入框数据时出错')
                            return
                        })
                    }
                    ...
                    axios.post( that.state.url + '/xmis/LMJcAction/GetJcComplishSign.do?jcid='+neededitemlist[i].layoverJcId).then(complishresponse =>{
                        ...
                        that._saveDataFile(userid+':'+taskid,'jccomplish-'+jcid,JSON.stringify(complishresponse.data),'拉取完工签名')
                        if(i == neededitemlist.length-1 ){
                    ...
                    axios.post( that.state.url + '/xmis/LMJcAction/VerifyJcComplishSign.do?jcid='+neededitemlist[i].layoverJcId).then(cancomplishresponse =>{
                        ...
                        that._saveDataFile(userid+':'+taskid,'jccomplishverify-'+jcid,JSON.stringify(cancomplishresponse.data),'拉取是否能完工签名')
                    ...
                    axios.post( that.state.url + '/xmis/LMJcAction/VerifySigner.do?jcid='+neededitemlist[i].layoverJcId+'&signtype=accomplishedSign&userid='+userid).then(canmodifycomplishresponse =>{
                        ...
                        that._saveDataFile(userid+':'+taskid,'jccomplishmodifyverify-'+jcid,JSON.stringify(canmodifycomplishresponse.data),'拉取是否能修改完工')
                    }).catch(err =>{
    ...
    _removeDownloaded = (userid,taskid)=>{
        ...
        console.log('删除:' + userid+'/'+taskid);
        SoraModule.removeDirectory(userid+'/'+taskid,(success)=>{
            console.log('删除:' + userid+'/'+taskid);
                ...
                <WebView
                    ...
                    onMessage={(msg) => {
                        ...
                        if (dataArr[0] === 'preview') {
                            ...
                            that._removeDownloaded(that.state.userId,taskid,(data)=>{
                                console.log('上传任务 -> 清空已下载数据 -> '+ data)
                            },err=>{
                                console.log('上传任务 -> 清空已下载数据失败')
                            });
                            that._getAllDataForKey(that.state.userId+':offlinepost:'+taskid,(ret)=>{
                                console.log('task:'+taskid+'下的签名')
                                console.log(ret);
                                if(ret.length===0){
                                ...
                                SoraModule.removeDirectory(that.state.userId+'/offlinepost/'+taskid,(data)=>{
                                    console.log(that.state.userId+'/offlinepost/'+taskid)
                                    console.log(data)
                                },err=>{
                                    console.log(that.state.userId+'/offlinepost/'+taskid)
                                    console.log(err)
                            ...
                            that._getAllDataForKey(that.state.userId+':offlineinput:'+taskid,ret=>{
                                console.log(ret);
                                if(ret.length===0){
                                ...
                                for(let i=0;i<ret.length;i++){
                                    ...
                                    let temp =  JSON.parse(ret[i])
                                    let par = '?jcid='+temp.jcid+'&dbid='+temp.dbid+'&value='+temp.value+'&userid='+temp.userid;
                                    if(temp.validator!=undefined){
                                        par+='&validator='+temp.validator
                                    }
                                    if(temp.inputvalueopt!=undefined){
                                        par+='&inputvalueopt='+temp.inputvalueopt
                                    }
                                    console.log('参数为'+par)
                                ...
                                SoraModule.removeDirectory(that.state.userId+'/offlineinput/'+taskid,(data)=>{
                                    console.log(that.state.userId+'/offlineinput/'+taskid)
                                    console.log(data)
                                },err=>{
                                    console.log(that.state.userId+'/offlineinput/'+taskid)
                                    console.log(err)
